...
router.route('/add').post((req, res) => {
	...
	newProject.save().then(() => res.json(newProject)).catch((err) => res.status(400).json('Error ' + err));
});
...
router.route('/update/:id').patch((req, res) => {
	...
	Project.findOneAndUpdate({ _id: id }, { $set: opsObj }, { upsert: true, new: true }, (err, doc) => {
